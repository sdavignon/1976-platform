...
3. Use the [migration tools](MIGRATION-TOOLS.md) to review schemas and plan a preserved-ID import. Preserve IDs, dates, ownership, account relationships, credits, orders, provider IDs and audit history. Normal create/bulkCreate rejects server-managed fields intentionally; the trusted migration API is separate. Verify counts and samples before switching traffic.
...
8. Configure the [live subscription and agent runtime](RUNTIME.md) for entity subscriptions and agent conversations. Supply responder adapters and port connector APIs/tool permissions separately. Snapshot delivery coalesces intermediate changes; it is not a durable business-event queue.
